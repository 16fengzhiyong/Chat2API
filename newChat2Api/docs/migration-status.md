...
- Built-in provider seed data is limited to migrated providers: Z.ai and Qwen AI.
...
- Redesigned web admin shell for dashboard, migrated providers, accounts, API keys, model mappings, sessions, prompts/tool calling, logs, and settings.
...
- Provider maintenance service for clear-chat / credits / model-refresh entry points, including real clear-chat calls for Qwen AI and Z.ai where credentials are available.
...
The current backend contains a provider adapter registry and built-in provider forwarder foundation. Z.ai and Qwen AI overseas are the completed private-provider migration targets for now. Only migrated providers are seeded and shown in the admin UI / reporter client. Other provider migrations are paused or skipped unless explicitly reprioritized:

- DeepSeek request format, token validation, stream parsing, clear chat.
- GLM refresh token validation, signing/header details, stream parsing, clear chat. Migration is intentionally skipped for now.
- Kimi gRPC-like request format, JWT validation, stream parsing. Migration is intentionally skipped for now.
...
- MiniMax JWT + realUserID behavior, signing, credits API, clear chat. Migration is intentionally skipped for now.
- Mimo multi-cookie validation, query format, stream parsing, clear chat. Migration is intentionally skipped for now.
...
- DeepSeek private migration is paused.
- Perplexity, Qwen domestic, MiniMax, GLM, Kimi, and Mimo migrations are intentionally skipped for now.
...
- Backend Java uses Gradle. `gradlew.bat build` passed.
- Admin Web production build passed with `npm run build`.
